...
def rotate_point(point, theta, axis):
    ...
    return np.dot(rotation_matrix, point) # R x point : multiplication from left side -> rotation of a point with respect to a fixed coordinate system
    # return np.dot(point, rotation_matrix) # point x R : multiplication from right side -> rotation of a point with respect to a moving coordinate system
...
def animate_rotation_comparison(axis='z', duration=10, fps=30):
    ...
    # Initial point different for each axis
    if axis == 'x':
    ...
    # Set up a static view based on rotation axis
    if axis == 'x':
        elev, azim = 20, -80 
    elif axis == 'y':
        elev, azim = 20, 10   
    ...
    for ax in [ax1, ax2]:
        ...
        # Set equal aspect ratio for the 3D plots
...
def main():
    ...
    # Compare numerical results for different axes
    for axis in ['x', 'y', 'z']:
